export { emojiRegex, MessageLink, MemberMention, MemberUsernameHash, unicodeEmojiRegex }

const emojiRegex = /:(\w+):(\d+)?/
const MessageLink = /https?:\/\/\w+\.com\/channels\/(\d+)\/(\d+)\/(\d+)/
const MemberMention = /<@!?(\d+)>/
const MemberUsernameHash = /(.+)\n?#(\d{4})/
